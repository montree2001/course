...
// หน้าสำหรับเพิ่มรายวิชาใหม่

// กำหนดค่าตัวแปรที่ใช้ในหน้านี้
$pageTitle = 'เพิ่มรายวิชาใหม่';
$currentPage = 'courses';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            // เตรียมคำสั่ง SQL
            $stmt = $pdo->prepare("
                INSERT INTO courses (course_code, course_name, theory_hours, practice_hours, credit_hours, created_at, updated_at)
                VALUES (:course_code, :course_name, :theory_hours, :practice_hours, :credit_hours, NOW(), NOW())
            ");
            ...
            if ($result) {
                ...
                $_SESSION['success_message'] = 'เพิ่มรายวิชาใหม่เรียบร้อยแล้ว';
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">เพิ่มรายวิชาใหม่</li>
    </ol>
</nav>

<!-- Page Heading -->
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">เพิ่มรายวิชาใหม่</h1>
</div>
    ...
    <div class="col-xl-12 col-lg-12">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">กรอกข้อมูลรายวิชา</h6>
            </div>
            <div class="card-body">
                <form action="add.php" method="post" id="courseForm">
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" id="course_code" name="course_code" value="<?php echo isset($_POST['course_code']) ? htmlspecialchars($_POST['course_code']) : ''; ?>" required>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control" id="theory_hours" name="theory_hours" min="0" value="<?php echo isset($_POST['theory_hours']) ? htmlspecialchars($_POST['theory_hours']) : '0'; ?>">
                        </div>
                        <div class="col-md-4">
                            <label for="practice_hours" class="form-label">จำนวนชั่วโมงปฏิบัติ</label>
                            <input type="number" class="form-control" id="practice_hours" name="practice_hours" min="0" value="<?php echo isset($_POST['practice_hours']) ? htmlspecialchars($_POST['practice_hours']) : '0'; ?>">
                        </div>
                        <div class="col-md-4">
                            <label for="credit_hours" class="form-label">จำนวนหน่วยกิต <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="credit_hours" name="credit_hours" min="1" value="<?php echo isset($_POST['credit_hours']) ? htmlspecialchars($_POST['credit_hours']) : '3'; ?>" required>
                        </div>
                    </div>
